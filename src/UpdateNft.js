...
const UpdateNft = () => {
  ...
  useEffect(() => {
    if (nftData && nftData.image) {
      // Check if image is already a data URL (base64)
      if (typeof nftData.image === 'string' && nftData.image.startsWith('data:')) {
        // For base64 data URLs, don't try to fetch - just use it directly
        setNftImage(nftData.image);
      } else {
        // For regular image URLs, fetch the blob
