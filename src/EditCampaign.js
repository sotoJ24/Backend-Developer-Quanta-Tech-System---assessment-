...
const EditCampaign = () => {
  ...
  const handleAddCategory = (category) => {
    if (category) {
      ...
      setAvailableFieldsForDropdown(categoryFields);
      setFieldToAddDefaultValue(categoryFields[0]?.question);
    } else {
  ...
  useEffect(() => {
    const loadCampaign = async () => {
      // TEST MODE: Load campaign from localStorage for test user
      if (user?._id === 'test-user-123') {
        try {
          const savedCampaigns = localStorage.getItem('test-campaigns');
          if (savedCampaigns) {
            const parsedCampaigns = JSON.parse(savedCampaigns);
            const tempCampaign = parsedCampaigns.find(c => c._id === id);
            if (tempCampaign) {
              setCampaignData(tempCampaign);
              if (tempCampaign.ends_at) setDate(new Date(tempCampaign.ends_at));
              if (tempCampaign.keyword_ids && tempCampaign.keyword_ids.length > 0) {
                for (let keyId of tempCampaign.keyword_ids) {
                  getKeywordById(keyId)
                    .then((res) => {
              ...
              // Handle cover and logo images
              if (tempCampaign.cover) {
                if (typeof tempCampaign.cover === 'string' && tempCampaign.cover.startsWith('data:')) {
                  // It's a base64 data URL - convert to blob
                  fetch(tempCampaign.cover)
                    .then(res => res.blob())
                    .then(blob => setCover(blob))
                    .catch(err => console.error('Error converting cover to blob:', err));
                } else if (tempCampaign.cover !== '') {
                  await getImageFields(`${process.env.REACT_APP_API}/uploads/${tempCampaign.cover}`, 'cover');
                }
              }
              
              if (tempCampaign.logo) {
                if (typeof tempCampaign.logo === 'string' && tempCampaign.logo.startsWith('data:')) {
                  // It's a base64 data URL - convert to blob
                  fetch(tempCampaign.logo)
                    .then(res => res.blob())
                    .then(blob => setLogo(blob))
                    .catch(err => console.error('Error converting logo to blob:', err));
                } else if (tempCampaign.logo !== '') {
              ...
              console.error('Campaign not found in test data');
              navigate('/dashboard/campaigns');
            }
          } else {
            console.error('No campaigns found in localStorage');
            navigate('/dashboard/campaigns');
          }
        } catch (error) {
